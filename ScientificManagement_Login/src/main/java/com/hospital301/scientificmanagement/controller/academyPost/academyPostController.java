...
public class academyPostController extends BaseController
{
	...
	public Object getList(@RequestBody String requestPayload) throws Exception
	{
		...
		Object object=  this.baseGetList(null, TableNameEnum.PROJECTACADEMYPOST.getName(),conditionMap);
 		if(null == object)
 			return null;
	...
	{
		Object object = this.baseGetListByUser(TableNameEnum.PROJECTACADEMYPOST.getName(), this.getConditionMap(requestPayload), this.getByUserMap());
		if(null == object)
 			return null;
	...
	{
		Object object = this.baseGetListByOrg(TableNameEnum.PROJECTACADEMYPOST.getName(), this.getConditionMap(requestPayload), this.getByOrgMap());
		if(null == object)
 			return null;
 		return JsonUtil.objectToJson(object);
	}
	...
	public Object add(@RequestBody String requestPayload) throws Exception
	{
		...
		{
			ProjectAcademyPost projectAcademyPost = (ProjectAcademyPost) requestAdd(requestPayload,ProjectAcademyPost.class,this.getAssociationTableJsonNode(),TypeEnum.ACADEMY.getName());
...
//			return projectAcademyPost;
	...
	public Object get(@RequestBody String requestPayload) throws Exception
	{
		...
		if (null != requestPayloadMap && requestPayloadMap.size()>0) {
			...
			return this.requestGet("project_academy_post", conditionMap, this.getAssociationTableJsonNode(), TypeEnum.ACADEMY.getName());
		}
		return null;
	}
	...
	public Object delete(@RequestBody String requestPayload) throws Exception
	{
		if(null != requestPayload)
		{
			ProjectAcademyPost projectAcademyPost = (ProjectAcademyPost) JsonUtil.JsonNodeToObject(requestPayload,"txnBodyCom",ProjectAcademyPost.class);
			if(null ==projectAcademyPost)
				return "从json获取数据为空";
			this.basedelete(TableNameEnum.PROJECTACADEMYPOST.getName(),this.getAssociationTableJsonNode(),TypeEnum.ACADEMY.getName(),projectAcademyPost.getId());
			return projectAcademyPost;
		}
		return null;
	}
