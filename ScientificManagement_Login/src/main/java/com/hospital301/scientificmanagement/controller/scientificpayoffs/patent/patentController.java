...
public class patentController extends BaseController
{
	...
	public Object  getRewardList(@RequestBody String requestPayload) throws Exception
	{
		...
		return this.baseGetList(null, TableNameEnum.PROJECTPATENT.getName(),conditionMap);
	...
	{
		return this.baseGetListByUser(TableNameEnum.PROJECTPATENT.getName(), this.getConditionMap(requestPayload), this.getByUserMap());
	}
	...
	{
		return this.baseGetListByOrg(TableNameEnum.PROJECTPATENT.getName(), this.getConditionMap(requestPayload), this.getByOrgMap());
	...
	public Object add(@RequestBody String requestPayload) throws Exception 
	{
		...
		{
			ProjectPatent projectPatent = (ProjectPatent) requestAdd(requestPayload,ProjectPatent.class,this.getAssociationTableJsonNode(),TypeEnum.PATENT.getName());
	...
	public Object get(@RequestBody String requestPayload) throws Exception
	{
		...
		if (null != requestPayloadMap && requestPayloadMap.size()>0) {
			...
			return this.requestGet(TableNameEnum.PROJECTPATENT.getName(), conditionMap, this.getAssociationTableJsonNode(), TypeEnum.PATENT.getName());
		}
		return null;
	}
	...
	{	
		if(null != requestPayload)
		{
			ProjectPatent projectPatent = (ProjectPatent) JsonUtil.JsonNodeToObject(requestPayload,"txnBodyCom",ProjectPatent.class);
			if(null ==projectPatent)
				return "从json获取数据为空";
			this.basedelete(TableNameEnum.PROJECTPATENT.getName(),this.getAssociationTableJsonNode(),TypeEnum.PATENT.getName(),projectPatent.getId());
			return projectPatent;
		}
		return "接受前台数据为空";
	}
